...
T_2way_measure = 500e-6 
T_TX_Postfix_gap = 4e-6
T_TX_Advance = 10e-6
...
if(ueid == "A"):
    ...
    port_udpsock = 19199
    timing_correction = int(-(T_2way_measure/2)*samp_rate)/samp_rate
else:
    print("Invalid UEID!")
    exit()
...
usrp = uhd.usrp.MultiUSRP("addr="+usrp_addr+", clock_source=external,time_source=external, master_clock_rate=250e6")
...
agc_controller = AGC_Controller(tgt_BB_power_dBc=-10, max_gain_dB=60, min_gain_dB=0)
