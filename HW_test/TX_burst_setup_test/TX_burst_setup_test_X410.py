...
# RF FE parameter feedback
rx_samp_rate_readback = usrp.get_rx_rate(0)
tx_samp_rate_readback = usrp.get_tx_rate(0)
print(f"Readback TX rate: {tx_samp_rate_readback} MHz, RX rate: {rx_samp_rate_readback} MHz.")
...
n_frames_per_set = 100000
# n_frames_per_set = 10000
iframes_in_current_set = 0
i_set = 0
tx_gap_postfix_time = 100e-6
# tx_gap_postfix_times = list(np.arange(0, 50, 25)*1e-6)
tx_gap_prefix_times = list(np.float64([0])*1e-6)
n_sets = len(tx_gap_prefix_times)
nframes_to_wait_init = int(round(t_tx_advance*samp_rate/nticks_rxb*2))
nframes_to_wait = 1000

tx_buffer[0, :] = 0
tx_buffer[0, int(round(tx_gap_prefix_times[i_set]*samp_rate)):nticks_txb-int(round(tx_gap_postfix_time*samp_rate))] = 0.805
        ...
        if(pos_frame_rx == nticks_rxb):
            ...
            if(nframes_to_wait == 0): # Wait for the TX-RX delay
                ...
                if(iframes_in_current_set == 0):
                    ...
                    current_settings_iq_envelope_max[i_set:] = np.abs(rx_frame_buf[0, :])
                ...
                if(iframes_in_current_set == n_frames_per_set):
                    ...
                    if(i_set == n_sets):
                        ...
                        # i_set = 0
                    tx_buffer[0, :] = 0
                    tx_buffer[0, int(round(tx_gap_prefix_times[i_set]*samp_rate)):nticks_txb-int(round(tx_gap_postfix_time*samp_rate))] = 0.75
                    nframes_to_wait = nframes_to_wait_init
                    print(tx_gap_prefix_times[i_set])
...
np.savetxt("temp20250721_max.csv", current_settings_iq_envelope_max, delimiter=",")
np.savetxt("temp20250721_min.csv", current_settings_iq_envelope_min, delimiter=",")
np.savetxt("temp20250721_avg.csv", current_settings_iq_envelope_avg, delimiter=",")
...
for i_set in range(n_sets):
    print(tx_gap_prefix_times[i_set])
    plt.plot(current_settings_iq_envelope_min[i_set, :])
    plt.plot(current_settings_iq_envelope_max[i_set, :])
    plt.plot(current_settings_iq_envelope_avg[i_set, :])
    plt.show()