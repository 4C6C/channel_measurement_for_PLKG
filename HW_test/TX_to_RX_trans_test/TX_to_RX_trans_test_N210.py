...
usrp = uhd.usrp.MultiUSRP("addr="+usrp_addr+", clock_source=internal,time_source=internal, master_clock_rate=100e6")
...
bb_if_samples = np.zeros((1, nticks_txb), dtype=np.complex64)
digi_if_freq = 0.25
bb_if_samples[0, :] = 0.9*np.exp(2j*np.pi*digi_if_freq*np.arange(np.size(tx_buffer)))
...
distributed_current_settings_iq_envelope_max = [np.zeros((nticks_rxb), dtype=np.float64)]*n_sets
distributed_current_settings_iq_envelope_min = [np.zeros((nticks_rxb), dtype=np.float64)]*n_sets
distributed_current_settings_iq_envelope_avg = [np.zeros((nticks_rxb), dtype=np.float64)]*n_sets
distributed_current_settings_iq_envelope_square_avg = [np.zeros((nticks_rxb), dtype=np.float64)]*n_sets
for i_set in range(n_sets):
    distributed_current_settings_iq_envelope_max[i_set] = np.zeros((nticks_rxb), dtype=np.float64)
    distributed_current_settings_iq_envelope_min[i_set] = np.zeros((nticks_rxb), dtype=np.float64)
    distributed_current_settings_iq_envelope_avg[i_set] = np.zeros((nticks_rxb), dtype=np.float64)
    distributed_current_settings_iq_envelope_square_avg[i_set] = np.zeros((nticks_rxb), dtype=np.float64)

i_set = 0
tx_buffer[0, :] = 0
tx_buffer[0, int(round(tx_gap_prefix_time*samp_rate)):nticks_txb-int(round(tx_gap_postfix_times[i_set]*samp_rate))] = bb_if_samples[0, int(round(tx_gap_prefix_time*samp_rate)):nticks_txb-int(round(tx_gap_postfix_times[i_set]*samp_rate))]
    ...
    if nsamps_received:
        ...
        if(pos_frame_rx == nticks_rxb):
            ...
            if(nframes_to_wait == 0): # Wait for the TX-RX delay
                ...
                rx_frame_buf[0, :] = signal.fftconvolve(rx_frame_buf[0, :], bpf_fir_025fs, mode='same')
                if(iframes_in_current_set == 0):
                    # Firdt sample ih this set
                    distributed_current_settings_iq_envelope_max[i_set][:] = np.abs(rx_frame_buf[0, :]) # Looks like it should be [i_set, :], but coincidentally, the current version works perfectly well, so let's leave it as is.
                    distributed_current_settings_iq_envelope_min[i_set][:] = np.abs(rx_frame_buf[0, :])
                    distributed_current_settings_iq_envelope_avg[i_set][:] = np.abs(rx_frame_buf[0, :])
                    distributed_current_settings_iq_envelope_square_avg[i_set][:] = np.square(np.abs(rx_frame_buf[0, :]))
                else:
                    distributed_current_settings_iq_envelope_max[i_set][:] = np.maximum(np.abs(rx_frame_buf[0, :]), distributed_current_settings_iq_envelope_max[i_set][:])
                    distributed_current_settings_iq_envelope_min[i_set][:] = np.minimum(np.abs(rx_frame_buf[0, :]), distributed_current_settings_iq_envelope_min[i_set][:])
                    distributed_current_settings_iq_envelope_avg[i_set][:] = distributed_current_settings_iq_envelope_avg[i_set][:] + np.abs(rx_frame_buf[0, :])
                    distributed_current_settings_iq_envelope_square_avg[i_set][:] = distributed_current_settings_iq_envelope_square_avg[i_set][:] + np.square(np.abs(rx_frame_buf[0, :]))
...
current_settings_iq_envelope_square_avg = np.zeros((n_sets, nticks_rxb), dtype=np.float64)
for i_set in range(n_sets):
    current_settings_iq_envelope_max[i_set, :] = distributed_current_settings_iq_envelope_max[i_set][:]
    current_settings_iq_envelope_min[i_set, :] = distributed_current_settings_iq_envelope_min[i_set][:]
    current_settings_iq_envelope_avg[i_set, :] = distributed_current_settings_iq_envelope_avg[i_set][:]
    current_settings_iq_envelope_square_avg[i_set, :] = distributed_current_settings_iq_envelope_square_avg[i_set][:]

current_settings_iq_envelope_avg /= n_frames_per_set
current_settings_iq_envelope_square_avg /= n_frames_per_set

dataname = "result_TX_ro_RX_trans_N210"
