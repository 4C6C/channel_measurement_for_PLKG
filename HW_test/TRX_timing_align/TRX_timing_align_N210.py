...
#     print("Invalid UEID!")
#     exit()
tx_timing_calibration_nsample = 0
...
i_set = 0
tx_gap_postfix_time = 50e-6
# tx_gap_postfix_times = list(np.arange(0, 50, 25)*1e-6)
tx_gap_prefix_times = list(np.float64([100])*1e-6)
n_sets = len(tx_gap_prefix_times)
nframes_to_wait_init = int(round(t_tx_advance*samp_rate/nticks_rxb*2))
nframes_to_wait = 1000

tx_buffer[0, :] = 0
tx_buffer[0, int(round(tx_gap_prefix_times[i_set]*samp_rate)):nticks_txb-int(round(tx_gap_postfix_time*samp_rate))] = bb_if_samples[0, int(round(tx_gap_prefix_times[i_set]*samp_rate)):nticks_txb-int(round(tx_gap_postfix_time*samp_rate))]
            ...
            if(nframes_to_wait == 0): # Wait for the TX-RX delay
                ...
                if(iframes_in_current_set == n_frames_per_set):
                    ...
                    if(i_set == n_sets):
                        ...
                        # i_set = 0
                    tx_buffer[0, :] = 0
                    tx_buffer[0, int(round(tx_gap_prefix_times[i_set]*samp_rate)):nticks_txb-int(round(tx_gap_postfix_time*samp_rate))] = 0.81
...
dataname = "result_TRX_timing_align_N210"
